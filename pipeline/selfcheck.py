...
from src.cost import Ledger, Usage, update_ledger
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    def run_once(input_tokens: int = 50_000) -> object:
        ledger = Ledger()
        ledger.record(Usage(model="claude-sonnet-5", input_tokens=input_tokens))
        return update_ledger(ledger, state)

    first = run_once()  # $0.15
    assert abs(first.run_cost_usd - 0.15) < 1e-9, first.run_cost_usd
    assert abs(first.spent_usd - 0.15) < 1e-9
    assert abs(first.average_run_usd - 0.15) < 1e-9
    assert first.runs_recorded == 1

    second = run_once(100_000)  # $0.30
    assert abs(second.spent_usd - 0.45) < 1e-9, second.spent_usd
    assert abs(second.average_run_usd - 0.225) < 1e-9, second.average_run_usd
    assert second.runs_recorded == 2
    print("ledger accumulation OK")

    # 平均は直近 AVERAGE_WINDOW 回だけを見る（古い実行を引きずらない）
    from src.cost import AVERAGE_WINDOW

    for _ in range(AVERAGE_WINDOW):
        latest = run_once(100_000)
    assert abs(latest.average_run_usd - 0.30) < 1e-9, latest.average_run_usd
    print("average window OK")

    # 状態ファイルが壊れていても実行は止めない
